...
describe("Digitoken", () => {
    ...
    before(async () => {
        ...
        digitoken = await Digitoken.deploy();
    ...
    describe("Mint", () => {
        it("should not allow minter to mint tokens when not whitelisted yet", async () => {
            ...
            expect(await digitoken.connect(owner).hasRole(MINTER, minter), `minter[${minterAddress}] is MINTER`).to.be.false;

            await digitoken.connect(owner).grantRole(MINTER, minterAddress);
            expect(await digitoken.connect(minter).hasRole(MINTER, minterAddress), `minter[${minterAddress}] has not MINTER by owner[${ownerAddress}]`).to.be.true;

            await expect(digitoken.connect(minter).mint(user1Address, amount)).to.be.revertedWith("Sender has not whitelisted");
        });

        it("should allow minter to mint tokens", async () => {
            const amount = ethers.parseEther("100");

            expect(await digitoken.connect(owner).hasRole(WHITE_LISTED, minter), `minter[${minterAddress}] is WHITELISTED`).to.be.false;

            await digitoken.connect(owner).grantRole(WHITE_LISTED, minterAddress);

            expect(await digitoken.connect(minter).hasRole(MINTER, minterAddress), `minter[${minterAddress}] has not MINTER by owner[${ownerAddress}]`).to.be.true;
            expect(await digitoken.connect(minter).hasRole(WHITE_LISTED, minterAddress), `minter[${minterAddress}] has not WHITELISTED by owner[${ownerAddress}]`).to.be.true;

            await digitoken.connect(minter).mint(user1Address, amount);
